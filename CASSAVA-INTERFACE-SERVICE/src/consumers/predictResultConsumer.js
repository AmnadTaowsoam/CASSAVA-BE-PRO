...
async function predictResult(msg, channel) {
    ...
    try {
        ...
        const averageResults = {
            phys0001: results.reduce((sum, r) => sum + parseFloat(r.phys0001), 0) / results.length,
            chem0010: results.reduce((sum, r) => sum + parseFloat(r.chem0010), 0) / results.length,
            chem0013: results.reduce((sum, r) => sum + parseFloat(r.chem0013), 0) / results.length
        };
        ...
        await interfacePool.query(
            'INSERT INTO interface.inspections (request_ref, insp_lot, plant, operation, sample_no, phys0001, chem0010, chem0013) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
            [requestRef, inslot, plant, operationno, sample_no, averageResults.phys0001, averageResults.chem0010, averageResults.chem0013]
        );
